...
class Exercise{
    constructor(name, unit, value, difficulty, link, targetMuscles, harderExercise, easierExercise, timePerRep, description){
        this.name = name
        this.unit = unit
        this.value = value
...
class Session {
    ...
    calculateCaloriesBurned(){ //TODO: @Aadi test this once workouts are added in the DB
        ...
        for (i = 0; i < this.workout.exerciseList.length; i++){
            ...
            if (this.workout.exerciseList[i].unit === "SECS"){
                ...
                exerciseDuration = this.completedStats[i].repsDone * this.workout.exerciseList[i].timePerRep
            }

            this.caloriesBurned += Math.floor((exerciseDuration/60) * (MET * 3.5 * userWeight) / 200)
        }
